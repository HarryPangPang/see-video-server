...
export const getUserProfile = async (ctx) => {
    ...
    const u = await db.get('SELECT id, COALESCE(username, email) AS name, avatar, bio, location, website, background, likes_public FROM users WHERE id = ?', [targetId]);
    ...
    ctx.body = { success: true, data: { id: u.id, name: u.name, avatar: u.avatar || null, bio: u.bio || null, location: u.location || null, website: u.website || null, background: u.background || null, followers: followers.cnt, following: following.cnt, likes_received: likes.cnt, is_following, likes_public: u.likes_public ?? 0 } };
};
